...
(defun aoc-eleven-one ()
  ...
  (let ((grid (aoc-parse-octopus-grid))
    ...
    (erase-buffer)
    (dotimes (n 100)
;;      (aoc-print-grid grid (format "Step %d" n))
      (let* ((step-flashes) '()
        ...
        (while (not (eq (length next-flashes) 0))
          ;; (insert (format "Flashing: %s" next-flashes))
          ;; (newline)
          ;; (insert (format "Inc Next: %s" neighbors))
          ;; (newline)
          ...
          (setq step-iteration (+ 1 step-iteration)))
;;          (aoc-print-grid grid (format "Step %d.%d" n step-iteration)))
;;        (aoc-print-grid grid (format "Step %d Final" n))
